...
public class DynamicBillboardChange : MonoBehaviour
{
    ...
    void GetAngle()
    {
        ...
        enemyDir.y = 0;
        float dotProdukt = Vector3.Dot(playerDir, enemyDir);
        if (dotProdukt < -0.5f && dotProdukt >= -1.0f)
        {
            ChangeSprite(0);

        }else if(dotProdukt > 0.5f && dotProdukt <=1.0f)
        {
            ChangeSprite(1);
        }
        else
        {
            ...
            playerRight.y = 0;
            dotProdukt = Vector3.Dot(playerRight, enemyDir);
            if (dotProdukt>=0)
            {
                ChangeSprite(2);
            }
